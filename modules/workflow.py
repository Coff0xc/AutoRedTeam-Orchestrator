...
class AutoWorkflow:
    ...
    def execute(self, target: str, options: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 生成攻击计划
            attack_plan = self.ai.generate_attack_plan(target, self.results.get("recon", {}))
    ...
    def _phase_recon(self, target: str, target_type: str, options: Dict) -> Dict[str, Any]:
        """信息收集阶段"""
        ...
        try:
            if target_type == "ip":
                ...
                nmap_result = self._execute_tool("nmap_scan", {"target": target, "timing": "T4"})
                ...
                # 子域名枚举
                subfinder_result = self._execute_tool("subfinder", {"domain": target})
                phase_result["results"]["subfinder"] = subfinder_result

                # DNS枚举
                dns_result = self._execute_tool("dns_enum", {"domain": target})
                ...
                whatweb_result = self._execute_tool("whatweb", {"target": target})
                phase_result["results"]["whatweb"] = whatweb_result

                waf_result = self._execute_tool("wafw00f", {"target": target})
    ...
    def _phase_vuln_scan(self, target: str, target_type: str, recon_result: Dict) -> Dict[str, Any]:
        """漏洞扫描阶段"""
        ...
        try:
            ...
            if target_type in ["ip", "domain", "url"]:
                ...
                nuclei_result = self._execute_tool(
                    "nuclei_scan", {"target": scan_target, "severity": "medium,high,critical"}
                )
                ...
                if nuclei_result.get("success"):
                    ...
                    self.results["vuln"] = {"vulnerabilities": vulns, "count": len(vulns)}
            ...
            if target_type == "url":
                ...
                nikto_result = self._execute_tool("nikto_scan", {"target": target})
...
class WorkflowBuilder:
    ...
    def add_step(
        self, tool_name: str, params: Dict = None, condition: str = None
    ) -> "WorkflowBuilder":
        """添加步骤"""
        self.steps.append({"tool": tool_name, "params": params or {}, "condition": condition})
        return self
