...
class CVESearchTool(BaseTool):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if vulns:
                    ...
                    cvss_v2 = (
                        metrics.get("cvssMetricV2", [{}])[0] if metrics.get("cvssMetricV2") else {}
                    )
            ...
            return {"success": False, "error": f"未找到CVE: {cve_id}"}
    ...
    def _search_keyword(
        self, keyword: str, vendor: str, product: str, limit: int
    ) -> Dict[str, Any]:
        """关键词搜索CVE"""
    ...
    def execute(self, params: Dict[str, Any], session_id: str = None) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(
                url, json={"query": search_query, "skip": 0, "size": limit}, timeout=self.timeout
            )
