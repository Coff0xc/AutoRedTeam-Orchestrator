...
class SSLScanTool(BaseTool):
    ...
    parameters: List[ToolParameter] = field(
        default_factory=lambda: [
            ...
            ToolParameter(
                "no_check_certificate", "boolean", "不验证证书", required=False, default=True
            ),
        ]
    )
    timeout: int = 120
        ...
        for line in lines:
            ...
            if "SSLv2" in line or "SSLv3" in line or "TLSv" in line:
                if "enabled" in line.lower():
                    ...
                    if proto:
                        result["protocols"].append({"version": proto.group(1), "enabled": True})
                        # 不安全协议警告
            ...
            for weak in weak_ciphers:
                if weak in line and ("Accepted" in line or "enabled" in line.lower()):
                    result["ciphers"].append({"cipher": line, "weak": True})
                    break
            else:
                if "Accepted" in line:
                    result["ciphers"].append({"cipher": line, "weak": False})
        ...
        for line in output.split("\n"):
            for pattern, severity, cve in vuln_patterns:
                if re.search(pattern, line, re.IGNORECASE):
                    findings.append({"finding": line.strip(), "severity": severity, "cve": cve})
