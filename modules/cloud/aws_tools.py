...
class AWSEnumTool(BaseTool):
    ...
    def execute(self, params: Dict[str, Any], session_id: str = None) -> Dict[str, Any]:
        ...
        results = {"success": True, "profile": profile, "region": region, "resources": {}}

        services_to_check = [service] if service != "all" else ["s3", "ec2", "iam", "lambda", "rds"]
            ...
            for bucket in buckets.get("Buckets", [])[:10]:  # 限制数量
                ...
                try:
                    acl = self._run_aws_cli(
                        ["s3api", "get-bucket-acl", "--bucket", name], profile, region
                    )
    ...
    def _enum_ec2(self, profile: str, region: str) -> Dict:
        """枚举EC2实例"""
        try:
            instances = self._run_aws_cli(["ec2", "describe-instances"], profile, region)
    ...
    def _enum_lambda(self, profile: str, region: str) -> Dict:
        """枚举Lambda函数"""
        try:
            functions = self._run_aws_cli(["lambda", "list-functions"], profile, region)
    ...
    def _enum_rds(self, profile: str, region: str) -> Dict:
        """枚举RDS数据库"""
        try:
            instances = self._run_aws_cli(["rds", "describe-db-instances"], profile, region)
...
class S3ScannerTool(BaseTool):
    ...
    parameters: List[ToolParameter] = field(
        default_factory=lambda: [
            ...
            ToolParameter(
                "check_permissions", "boolean", "检查权限配置", required=False, default=True
            ),
        ]
    )
    timeout: int = 120
