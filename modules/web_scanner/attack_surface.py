...
class AttackSurfaceDiscovery:
    ...
    # 常见的敏感参数名（用于识别可能的注入点）
    SENSITIVE_PARAMS = {
        "id", "user", "username", "password", "pass", "pwd", "email", "token",
        "key", "secret", "api_key", "apikey", "auth", "session", "cookie",
        "file", "path", "url", "redirect", "next", "return", "goto", "dest",
        "callback", "cb", "ref", "referer", "page", "p", "q", "search", "query",
        "sort", "order", "filter", "type", "action", "cmd", "command", "exec",
        "debug", "test", "admin", "role", "access", "level", "privilege",
    }
