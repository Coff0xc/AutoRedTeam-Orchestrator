...
class FTPCheckTool(BaseTool):
    ...
    parameters: List[ToolParameter] = field(
        default_factory=lambda: [
            ...
            ToolParameter(
                "check_anonymous", "boolean", "检测匿名登录", required=False, default=True
            ),
            ...
            if "VULNERABLE" in output:
                if "MS12-020" in output:
                    result["vulnerabilities"].append(
                        {"id": "MS12-020", "name": "BlueKeep predecessor", "severity": "critical"}
                    )
        ...
        try:
            ...
            for line in result.stdout.split("\n"):
                if line.strip() and "=" in line:
                    parts = line.split("=", 1)
                    if len(parts) == 2:
                        entries.append({"oid": parts[0].strip(), "value": parts[1].strip()})
...
class LDAPEnumTool(BaseTool):
    ...
    parameters: List[ToolParameter] = field(
        default_factory=lambda: [
            ...
            ToolParameter("password", "string", "密码", required=False, default=""),
            ToolParameter(
                "search_filter", "string", "搜索过滤器", required=False, default="(objectClass=*)"
            ),
        ]
    )
    timeout: int = 120
