...
class HydraTool(BaseTool):
    ...
    parameters: List[ToolParameter] = field(
        default_factory=lambda: [
            ...
            ToolParameter(
                "password_list", "string", "密码字典", required=False, default=""
            ),  # 运行时动态获取
...
class MedusaTool(BaseTool):
    ...
    def _parse_output(self, output: str) -> List[Dict[str, str]]:
        ...
        for match in re.finditer(pattern, output, re.IGNORECASE):
            credentials.append(
                {"host": match.group(1), "username": match.group(2), "password": match.group(3)}
            )
...
class CrackMapExecTool(BaseTool):
    ...
    def execute(self, params: Dict[str, Any], session_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": "crackmapexec未安装，请运行: apt install crackmapexec",
            }
        except Exception as e:
            return {"success": False, "error": str(e)}
    ...
    def _parse_output(self, output: str, protocol: str) -> Dict[str, Any]:
        """解析CrackMapExec输出"""
        result = {"hosts": [], "credentials": [], "shares": [], "users": []}
        ...
        for line in output.split("\n"):
            ...
            if host_match:
                ...
                result["hosts"].append(
                    {"ip": host_match.group(1), "info": host_match.group(2), "status": status}
                )
