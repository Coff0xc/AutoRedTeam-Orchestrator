    ...
    def execute(self, params: Dict[str, Any], session_id: str = None) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                ["whois", target], capture_output=True, text=True, timeout=self.timeout
            )

            return {"success": True, "target": target, "raw_output": result.stdout}
...
class TheHarvesterTool(BaseTool):
    ...
    def execute(self, params: Dict[str, Any], session_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": "theHarvester未安装，请运行: apt install theharvester",
            }
        except Exception as e:
            return {"success": False, "error": str(e)}
...
class CensysLookupTool(BaseTool):
    ...
    parameters: List[ToolParameter] = field(
        default_factory=lambda: [
            ...
            ToolParameter(
                "api_secret", "string", "Censys API Secret", required=False, default=None
            ),
        ]
    )
    timeout: int = 60
    ...
    def execute(self, params: Dict[str, Any], session_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if re.match(ip_pattern, target):
                host = h.view(target)
                return {"success": True, "target": target, "type": "ip", "data": host}
            else:
                results = list(h.search(f"names: {target}", per_page=10))
                return {"success": True, "target": target, "type": "domain", "results": results}
