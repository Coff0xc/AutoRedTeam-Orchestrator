...
class JWTTester(BaseAPITester):
    ...
    WEAK_SECRETS = [
        ...
        'jwt_secret', 'api_secret', 'token_secret', 'auth_secret',
        'supersecret', 'mysecret', 'admin', 'test', 'demo',
        'changeme', 'letmein', 'welcome', 'passw0rd',
        # 常见密钥名
        'jwt-secret', 'jwt_key', 'secret_key', 'SECRET_KEY',
        'JWT_SECRET', 'API_KEY', 'app_secret', 'application_secret',
        # 开发环境常见
        'development', 'production', 'staging', 'testing',
        'dev_secret', 'prod_secret', 'local_secret',
        # 字符串变体
        '', ' ', 'null', 'undefined', 'none', 'nil',
        # 常见短密钥
        'a', 'abc', '1234', 'abcd1234', 'qwerty',
        # JWT相关
        'HS256-secret', 'hmac-secret', 'your-256-bit-secret',
        'your-secret-key', 'my-secret-key', 'super-secret-key',
    ]
        ...
        for alg in none_variants:
            ...
            if self._verify_token_accepted(none_token):
                result = self._create_result(
                    ...
                    title='JWT None算法漏洞',
                    description=(
                        f'服务端接受alg="{alg}"的令牌，攻击者可以伪造任意JWT令牌'
                        '绕过身份验证。'
                    ),
                    evidence={
    ...
    def test_kid_injection(self) -> Optional[APITestResult]:
        ...
        if vulnerable_payloads:
            result = self._create_result(
                ...
                title='JWT KID注入漏洞',
                description=(
                    'JWT的kid参数存在注入漏洞，可能导致路径遍历、SQL注入或命令注入。'
                ),
                evidence={
    ...
    def test_signature_stripping(self) -> Optional[APITestResult]:
        ...
        if self._verify_token_accepted(stripped_token):
            result = self._create_result(
                ...
                evidence={
                    'stripped_token': stripped_token[:50] + '...'
                },
                remediation=(
                    '1. 服务端必须验证JWT签名\n'
                    '2. 拒绝签名为空或无效的令牌\n'
