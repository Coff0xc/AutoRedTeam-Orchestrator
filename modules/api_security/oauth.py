...
class OAuthTester(BaseAPITester):
    ...
    COMMON_ENDPOINTS = {
        ...
        'openid_config': ['/.well-known/openid-configuration', '/.well-known/oauth-authorization-server'],
    }
    ...
    def test_scope_manipulation(self) -> Optional[APITestResult]:
        ...
        if accepted_scopes:
            result = self._create_result(
                ...
                title='OAuth Scope操纵',
                description=(
                    f'授权服务器接受了{len(accepted_scopes)}个可能的特权scope。'
                ),
                evidence={
    ...
    def test_token_leak_referrer(self) -> Optional[APITestResult]:
        ...
        try:
            ...
            if not referrer_policy or referrer_policy.lower() in ['unsafe-url', 'no-referrer-when-downgrade']:
                self._create_result(
                    vulnerable=False,
...
def quick_oauth_test(
    ...
    Returns:
        测试结果摘要
    """
    tester = OAuthTester(target, config={
        'client_id': client_id,
        'redirect_uri': redirect_uri
    })
    results = tester.test()
