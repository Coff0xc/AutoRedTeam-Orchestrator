...
class DecisionEngine:
    ...
    def _adjust_stealth_preference(self) -> None:
        """根据检测事件调整隐蔽性权重"""
        detection_count = sum(
            len(h.get('detection_events', []))
            for h in self._attack_history[-5:]
        )
        if detection_count > 3:
            self._weights['stealth'] = min(0.35, self._weights['stealth'] + 0.05)
            self._weights['success_probability'] = max(0.25, self._weights['success_probability'] - 0.025)
    ...
    def suggest_attack_paths(self) -> List[AttackPath]:
        ...
        service_ports = [p for p in open_ports if p.get('service') in ('ssh', 'ftp', 'mysql', 'smb', 'rdp', 'winrm')]
        if service_ports:
    ...
    def _categorize_vulns(self, findings: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        categories: Dict[str, List[Dict[str, Any]]] = {
            'rce': [], 'sqli': [], 'ssrf': [], 'xxe': [], 'lfi': [],
            'ssti': [], 'xss': [], 'auth': [], 'upload': [], 'deserialize': [], 'other': [],
        }
    ...
    def _build_sqli_path(self, vulns: List[Dict[str, Any]]) -> AttackPath:
        ...
        can_rce = any(
            v.get('stacked_queries') or 'stacked' in v.get('type', '').lower()
            for v in vulns
        )
