...
class HumanBehaviorSimulation(EvasionTechnique):
    ...
    async def apply(self, context: RequestContext, config: StealthConfig) -> RequestContext:
        ...
        self._request_history = [
            (ts, url) for ts, url in self._request_history 
            if now - ts < 60
        ]
        ...
        # 添加常见浏览器行为特征
        context.headers.setdefault('Accept', 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8')
...
class TrafficObfuscation(EvasionTechnique):
    ...
    async def apply(self, context: RequestContext, config: StealthConfig) -> RequestContext:
        ...
        context.url = urlunsplit((parts.scheme, parts.netloc, parts.path, new_query, parts.fragment))
        ...
        if status_code in (403, 429, 503):
            for indicator in waf_indicators:
                if indicator in body_lower:
                    ...
                    self.metrics.add_detection_event('blocked', f'Status {status_code}: {indicator}')
                    break
