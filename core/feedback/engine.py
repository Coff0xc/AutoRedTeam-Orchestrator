        ...
        **operation_kwargs
        ...
        return asyncio.run(
            self.execute_with_feedback(
                operation, detection_result, max_retries, **operation_kwargs
            )
        )
    ...
    def _build_adjustment_params(
        ...
        if strategy.adjustment_type == AdjustmentType.PROXY:
            ...
            if params.get('value_type') == 'random_ip':
                params['value'] = f'{random.randint(1,223)}.{random.randint(0,255)}.{random.randint(0,255)}.{random.randint(1,254)}'
