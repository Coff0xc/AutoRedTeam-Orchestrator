...
async def _run_scanner(
    ...
    try:
        ...
        from tools.vuln_tools import (
            sqli_detect, xss_detect, ssrf_detect,
            lfi_detect_standalone, redirect_detect, crlf_detect, nosql_detect
        )
                    ...
                    if result.get("crlf_vulns"):
                        for vuln in result["crlf_vulns"]:
                            point_findings.append({
                                ...
                                "severity": "MEDIUM" if "Response Splitting" not in vuln.get("type", "") else "HIGH",
                                "url": url,
                                "param": param,
                                "method": method,
                                "payload": vuln.get("payload", ""),
...
async def _verify_findings(
    ...
    vuln_type_map = {
        ...
        "Open Redirect": {"code": "redirect", "verifiable": False, "reason": "需人工确认重定向目标的安全性"},
        "CRLF Injection": {"code": "crlf", "verifiable": False, "reason": "需人工确认注入头部的影响"},
        "NoSQL Injection": {"code": "nosql", "verifiable": False, "reason": "需人工确认数据库类型和注入效果"},
    }
    ...
    return [
        {
            ...
            "parameters": {
                ...
                "include_js": {"type": "boolean", "description": "是否分析 JS 文件", "default": True},
            },
        },
        {
            "name": "web_scan",
            ...
            "parameters": {
                "url": {"type": "string", "description": "目标 URL", "required": True},
                "mode": {"type": "string", "description": "扫描模式 (quick/full)", "default": "quick"},
                "scan_types": {"type": "array", "description": "扫描类型列表", "default": None},
                "max_pages": {"type": "integer", "description": "最大爬取页面数", "default": 30},
                "verify_findings": {"type": "boolean", "description": "是否验证发现", "default": True},
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(f"  - 注入点: {discover_result.get('discovery', {}).get('injection_points_total', 0)}")
