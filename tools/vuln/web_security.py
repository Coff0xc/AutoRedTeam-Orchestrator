...
def csrf_detect(url: str, method: str = "POST") -> dict:
    ...
    try:
        ...
        if set_cookie:
            if (
                "samesite=strict" not in set_cookie.lower()
                and "samesite=lax" not in set_cookie.lower()
            ):
                vulns.append(
                    {
    ...
    return {
        ...
        "total": len(vulns),
        "recommendations": (
            ["实施CSRF Token机制", "设置Cookie的SameSite属性", "验证Origin/Referer头"]
            if vulns
            else []
        ),
    }
    ...
    try:
        ...
        for header_name, header_value in CACHE_POISON_HEADERS:
            try:
                ...
                resp = requests.get(
                    cache_buster, headers=headers, timeout=10, verify=get_verify_ssl()
                )
