...
def vuln_check(url: str) -> dict:
    ...
    try:
        resp = requests.get(
            url, headers={"Origin": "https://evil.com"}, timeout=5, verify=get_verify_ssl()
        )
    ...
    try:
        ...
        if missing_headers:
            vulns.append(
                {"type": "Missing Security Headers", "severity": "LOW", "headers": missing_headers}
            )
    except (requests.RequestException, OSError):
        logger.warning("Suppressed exception", exc_info=True)
    ...
    try:
        ...
        if "allow" in resp.headers:
            ...
            if dangerous:
                vulns.append(
                    {"type": "Dangerous HTTP Methods", "severity": "MEDIUM", "methods": dangerous}
                )
    except (requests.RequestException, OSError):
        logger.warning("Suppressed exception", exc_info=True)
...
def get_baseline_response(url: str, num_samples: int = 3) -> tuple:
    ...
    for _ in range(num_samples):
        try:
            start = time.time()
            resp = requests.get(
                url, timeout=GLOBAL_CONFIG["request_timeout"], verify=get_verify_ssl()
            )
...
def check_response_for_patterns(
    response_text: str, patterns: List[str], case_sensitive: bool = False
) -> Optional[str]:
    """检查响应文本中是否包含指定模式
