...
XXE_INDICATORS = [
    ...
    "cm9vdDo",
    "ZGFlbW9u",  # base64 encoded root:/daemon:
    "SSH-",
    "OpenSSH",
]
    ...
    for payload in XXE_PAYLOADS:
        try:
            if method.upper() == "POST":
                resp = requests.post(
                    url, data=payload, headers=headers, timeout=15, verify=get_verify_ssl()
                )
            else:
...
def deserialize_detect(url: str, param: str = None, method: str = "POST") -> dict:
    ...
    try:
        ...
        if method.upper() == "POST":
            ...
            resp = requests.post(
                url, data=data, headers=headers, timeout=10, verify=get_verify_ssl()
            )
