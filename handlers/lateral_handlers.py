...
def register_lateral_tools(mcp, counter, logger):
    ...
    async def lateral_wmi_query(
        ...
        result = wmi_query(
            target=target, username=username, password=password or "", wql=query, domain=domain
        )
    ...
    async def lateral_auto(
        ...
        if not lateral:
            return {"success": False, "target": target, "error": "无法找到可用的横向移动方法"}
    ...
    async def credential_spray(
        ...
        # 构建凭证列表 - 用户名和密码的笛卡尔积
        credentials_list = [
            Credentials(username=u, password=p, domain=domain) for u in usernames for p in passwords
        ]
        ...
        results = spray_credentials(
            targets=targets, credentials_list=credentials_list, methods=methods
        )
                    ...
                    valid_creds.append(
                        {
                            "target": target,
                            "username": (
                                creds.username if hasattr(creds, "username") else str(creds)
                            ),
