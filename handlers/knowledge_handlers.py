...
def _fail_knowledge_runtime(gate: Dict[str, Any], exc: Exception) -> None:
    complete_handler_runtime_action(
        gate,
        False,
...
def register_knowledge_tools(mcp, counter, logger):
    ...
    async def kg_store(
        ...
        runtime_gate = _gate_knowledge_runtime(
            "kg_store",
            {
            ...
            risk_level="high" if entity_type == "credential" else "moderate",
            requires_auth=True,
        )
        ...
        try:
            ...
            _fail_knowledge_runtime(runtime_gate, exc)
            raise
    ...
    async def kg_query(
        ...
        runtime_gate = _gate_knowledge_runtime(
            "kg_query",
            {"entity_type": entity_type, "severity": severity, "limit": limit},
        )
        if not runtime_gate["allowed"]:
            return blocked_handler_runtime_response(runtime_gate)
        ...
        return complete_handler_runtime_payload(
            runtime_gate,
            payload,
            summary_keys=["count", "entity_type"],
        )
    ...
    async def kg_attack_paths(
        ...
        try:
            ...
            if not source:
                return complete_handler_runtime_payload(
                    runtime_gate,
                    {"success": False, "error": "源实体不存在: %s" % source_id},
                )
            ...
            if not target:
                return complete_handler_runtime_payload(
                    runtime_gate,
                    {"success": False, "error": "目标实体不存在: %s" % target_id},
                )
