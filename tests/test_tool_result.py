...
class TestToolResult:
    ...
    def test_ok_with_metadata(self):
        """测试带元数据的成功返回"""
        result = ToolResult.ok(data={"count": 5}, duration=1.5, target="example.com")
        assert result.success is True
    ...
    def test_partial(self):
        """测试部分成功返回"""
        result = ToolResult.partial(data={"scanned": 8, "total": 10}, error="2 hosts unreachable")
    ...
    def test_timeout(self):
        ...
        result = ToolResult.timeout(
            error="Scan exceeded 60s limit", data={"partial_results": [1, 2, 3]}
        )
        assert result.success is False
    ...
    def test_error(self):
        """测试内部错误返回"""
        result = ToolResult.internal_error("Unexpected null pointer", error_type="InternalError")
        assert result.success is False
    ...
    def test_mcp_tool_pattern(self):
        ...
        async def port_scan(target: str, ports: list) -> dict:
            try:
                ...
                return ToolResult.timeout(error=f"Scan of {target} timed out").to_dict()
            except Exception as e:
