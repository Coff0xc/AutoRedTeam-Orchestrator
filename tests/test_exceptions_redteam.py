...
class TestPrivilegeEscalationExceptions:
    ...
    def test_privilege_escalation_error(self):
        ...
        exc = PrivilegeEscalationError(
            "提权失败", method="uac_bypass", current_level="medium", target_level="high"
        )
    ...
    def test_escalation_vector_not_found(self):
        """测试未找到提权向量异常"""
        exc = EscalationVectorNotFound("未找到可用的SUID二进制文件", method="suid")
    ...
    def test_insufficient_privilege(self):
        """测试权限不足异常"""
        exc = InsufficientPrivilege("需要管理员权限", current_level="user", target_level="admin")
    ...
    def test_token_manipulation_error(self):
        ...
        exc = TokenManipulationError(
            "无法复制Token", method="token_impersonation", details={"pid": 4, "error_code": 5}
        )
    ...
    def test_exception_to_dict(self):
        """测试异常序列化为字典"""
        exc = PrivilegeEscalationError("提权测试", method="test_method")
...
class TestExfiltrationExceptions:
    ...
    def test_exfiltration_error(self):
        """测试基础数据外泄异常"""
        exc = ExfiltrationError("外泄失败", channel="dns", destination="exfil.example.com")
    ...
    def test_channel_blocked(self):
        """测试通道被阻断异常"""
        exc = ChannelBlocked("DNS出站被防火墙拦截", channel="dns")
    ...
    def test_data_too_large(self):
        """测试数据过大异常"""
        exc = DataTooLarge("数据超出DNS通道容量", channel="dns", data_size=1000000, max_size=65535)
    ...
    def test_channel_connection_error(self):
        ...
        exc = ChannelConnectionError(
            "SMB连接被拒绝", channel="smb", destination="\\\\target\\share"
        )
    ...
    def test_encryption_required(self):
        """测试需要加密异常"""
        exc = EncryptionRequired("该通道要求数据加密", channel="https")
    ...
    def test_exception_to_dict(self):
        """测试异常序列化为字典"""
        exc = ExfiltrationError("外泄测试", channel="https")
...
class TestExceptionChaining:
    ...
    def test_exception_with_cause(self):
        ...
        exc = ChannelBlocked("外泄通道被阻断", channel="https", cause=original)
    ...
    def test_privilege_exception_chain(self):
        ...
        exc = InsufficientPrivilege("无法访问目标进程", cause=original)
